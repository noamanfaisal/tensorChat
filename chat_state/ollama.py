...
class OllamaChatState(BaseChatState):
    ...
    def add_message(self, role: str, content: str) -> None:
        if self.current_topic:
            message_number = len(self.current_topic["messages"]) + 1
            self.current_topic["messages"].append({
                "number": message_number,  # <-- Add numbering
                "role": role,
                "content": content
            })
    ...
    def _save_topic_atomic(self):
        ...
        # Add message numbers before saving
        numbered_messages = []
        for idx, msg in enumerate(self.current_topic["messages"], start=1):
            msg_with_number = msg.copy()
            msg_with_number["number"] = idx
            numbered_messages.append(msg_with_number)
        
        # Create a copy of current_topic to avoid modifying in-memory state
        topic_copy = self.current_topic.copy()
        topic_copy["messages"] = numbered_messages
        
        with open(tmp_filename, "w") as f:
            json.dump(topic_copy, f, indent=2)
